...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        foreach ($rows as $row) {
            if ($row->type === 'hidden' && $slug === 'sale-promotions') {
                continue;
            }
        ...
        if ($request->has('meta')) {
            $data->metaTags()->updateOrCreate([
                'meta_taggable_id' => $data->id
            ], [
        ...
        if ($slug != 'regions' && $slug !== 'category-services' && $slug !== 'category-child-price-list') {
            $data->tour3d()->sync(is_null($request->tour3d[0]) ? [] : $request->tour3d);
        }

        if ($slug === 'category-main-repair') {
            if ($request->has('partners')) {
                if (!is_null($request->partners[0])) {
                    $data->partners()->saveMany(Partner::whereIn('id', $request->partners)->get());
                }
            } elseif(isset($data->partners)) {
                foreach ($data->partners as $item) {
                    $item->category_main_repair_id = null;
                    $item->save();
                }
            }
        }

        if ($slug === 'category-repair' && $request->has('price_list')) {
            if (!is_null($request->price_list[0])) {
                $data->priceList()->saveMany(PriceList::whereIn('id', $request->price_list)->get());
            }
        } elseif(isset($data->priceList)) {
            if (count($data->priceList)) {
                foreach ($data->priceList as $item) {
                    $item->repair_category_id = null;
                    $item->save();
                }
            }
        }
