...
<div class="panel panel-bordered">
  ...
  @endif
  <input type="hidden" class="content-sort" data-sort="{{'4_' . $key}}" name="contents[{{'4_' . $key}}][id]" value="{{$post_content['id'] ?? 0}}">
  <div class="panel-body">
    <div class="row">
      <div class="form-group col-md-12">
        <label for="name">Заголовок h2</label>
        <input required="" type="text" class="form-control" name="contents[{{'4_' . $key}}][title]"
               value="{{$post_content['title'] ?? ''}}">
      </div>
      <div class="form-group col-md-12">
        <label for="name">Текст</label>
        <textarea required="" name="contents[{{'4_' . $key}}][text]" cols="30" rows="10" class="form-control ckeditor22"
                  id="{{$key}}_editor">{{$post_content['text'] ?? ''}}</textarea>
      </div>
      <div class="clone-wrapper">
        <label class="content-label-for-items">Динамик тексты с точками</label>
        @forelse( $post_content->texts ?? [] as  $item )
          <div class="toclone form-group">
            <div class="col-sm-9">
              <input type="text" class="form-control" name="contents[{{'4_' . $key}}][texts][]" value="{{$item}}"
                     required="">
            </div>
        ...
        @empty
          <div class="toclone form-group">
            <div class="col-sm-9">
              <input type="text" class="form-control" name="contents[{{'4_' . $key}}][texts][]" value="" required="">
            </div>
            <div class="col-sm-3 cloneya-buttons">
              <div class="clone">
                <a type="button" class="btn btn-xs btn-primary">
