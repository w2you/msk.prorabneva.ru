...
$(document).ready(function () {
    ...
    $('.lightbox', this).topbox({

        afterShowLightbox: function(){
            $(".topbox_content").append('<a href="#" class="topbox_close" tabindex="1" title="Close"></a>');
    ...
    $ ( "body" ).on ( "click" , ".topbox_skin_darkroom .topbox_content .topbox_close", function (e) {
        e.preventDefault();
        $(".topbox_skin_darkroom>.topbox_close").trigger("click");

    } );

    for (var i = 0; i < $("section.interior_design a.lightbox").length; i++ ){
        $("section.interior_design a.lightbox").eq(i).parent().find("a.lightbox").attr('title','1 из ' + $("section.interior_design a.lightbox").eq(i).parent().find("a.lightbox").length);
    }
    $ ( "section.interior_design a.lightbox" ).on ( "click" , function () {
        var $this = $(this);
        $this.parent().find("a.lightbox").attr('title','');
        $this.parent().find("a.lightbox").attr('title','1 из ' + $this.parent().find("a.lightbox").length);
        $this.parent().find("a.lightbox").attr('title', function(index){
            var start = index + 1;
            return   '' + start + ' из ' + $this.parent().find("a.lightbox").length;
        });
    } );

    $("section.page_blog a.lightbox , section.photo_building a.lightbox ").attr('title', function(index, title){
        var start = index + 1;
        return  title + '' + start + ' из ' + $(" a.lightbox").length;
    });

    $ ( ".scroll__prices" ).on ( "click" , function () {
        if (window.innerWidth > 1000){
            $ ( 'html, body' ).animate ( {
                scrollTop : ($( "section.price_list-section" ).offset ().top - 75 )
            } , 800 );
            ...
            $ ( 'html, body' ).animate ( {
                scrollTop : ($( "section.price_list-section" ).offset ().top - 100 )
            } , 800 );
        }
        return false;
    } );

    $ ( ".scroll__services" ).on ( "click" , function () {
        if (window.innerWidth > 1000){
            $ ( 'html, body' ).animate ( {
                scrollTop : ($ ( "section.services__prices" ).offset ().top - 75 )
            } , 800 );
        } else if(window.innerWidth < 820) {
            $ ( 'html, body' ).animate ( {
                scrollTop : ($ ( "section.services__prices" ).offset ().top - 75)
            ...
            $ ( 'html, body' ).animate ( {
                scrollTop : ($( "section.services__prices" ).offset ().top - 100 )
            } , 800 );
        }
        return false;
    } );

    $ ( "#popup_services_table .popup_block" ).on ( "click" , function ( e ) {
        e.stopPropagation ();
    } );

    $ ( ".open__table_popup" ).on ( "click" , function ( e ) {
        e.preventDefault ();
        $ ( "body" ).css ( "overflow" , "hidden" );
        if ( $(window).width() < 768 ){
            $("#popup_services_table .popup_block .popup__tables .table__tr .table__td").removeClass("active");
            for ( var i = 0; i <$("#popup_services_table .popup_block .popup__tables .table__tr").length; i++  ) {
                $("#popup_services_table .popup_block .popup__tables  .table__tr ").eq(i).find(".table__td").eq(1).addClass("active");
            }
        }else{
            $("#popup_services_table .popup_block .popup__tables .table__tr .table__td").removeClass("active");
        }
        $ ( "#popup_services_table" ).fadeIn ( function () {

        } );
    } );

    $ ( "#popup_services_table .close, #popup_services_table " ).on ( "click" , function ( e ) {
        e.preventDefault ();
        $ ( "#popup_services_table" ).fadeOut ( function () {
            $ ( "body" ).css ( "overflow" , "auto" );
        } );
    } );


    $('.select_city_bloc_link').on('click', function (e) {
        console.log('select_city_bloc_link');
        $("#regions_select").fadeIn(function () {
            $("body").css("overflow", "auto");
        });
    })

    $('.region_close').on('click', function (e) {
        $("#regions_select").fadeOut(function () {
            $("body").css("overflow", "auto");
        });
    })
    ...
    /*$('*[data-tag]').click(function(e)
    {
      ...
      if (tag.length)
      {
        yaCounter44756221.reachGoal(tag);
        ga('send', 'event', 'Heder', tag);
    ...
    $('section.repairs_slider .slider').owlCarousel({
        loop: $(".blog-carousel > .type-post").length <= 1 ? false : true,
        margin: 0,
        items: 1,
        lazyLoad: true,
        nav: $(".blog-carousel > .type-post").length <= 1 ? false : true,
        navText: false,
        mouseDrag: false,
        touchDrag: false,
        responsive: {
  ...
  */
    ...
    $("section.tur_3d .tur_slider .block .owl_slid").on("click", function () {
        var $this = $(this);
        $this.toggleClass("play_video");
    });
    /* $ ( "section.reviews .reviews_slider .owl-item" ).on ( "click" , function (e) {
         e.preventDefault();
         var $this = $ ( this );
    ...
    $('.maska_phone').mask('+7 (999) 999-99-99').click(function () {
    ...
    function init() {
        ...
        for (var i = 1; i < 70; i++) {
            var lat = getRandomArbitrary(55.571431, 55.903701);
            var lng = getRandomArbitrary(37.393953, 37.819885);
            myPlacemark_array[i] = new ymaps.Placemark([lat, lng], {
                    //  balloonContent: '<div class="map_content"> <p><span>м. Проспект  Просвещения </span></p> <div class="line"></div><p>Санкт-Петербург, пр. Луначарского, д. 15, корп. 1</p><p><a href="tel:+78124261015"><span>+7 (812) 426-10-15</span></a></p><div class="line"></div><p>пн-сб 10.00-20.00<br> воскресенье 10.00-18.00</p> </div>'
                },
                {
    ...
    function init_map_center() {
        if ($('div').is('#map_center')) {
            var coordinates = $('#map_center').attr('coordinates');
            var map_attr = coordinates.split(',');
            var lat = map_attr[0];
            var log = map_attr[1];
            console.log(lat,log);
        }
        ...
        var myPlacemark = new ymaps.Placemark([lat, log], {
                //      balloonContent: '<div class="map_content"> <p><span>м. Площадь  Восстания </span></p> <div class="line"></div><p>Санкт-Петербург, ул. 2-я Советская д. 7</p><p><a href="tel:+78124261015"><span>+7 812 439-16-17</span></a></p><div class="line"></div><p>пн-пт 9.00-20.00 <br> сб-вс 10.00-20.00</p> </div>'
            },
            {
                iconLayout: 'default#image',
    ...
    function init() {
        ...
        var myPlacemark1 = new ymaps.Placemark([59.852253, 30.217503], {
                balloonContent: '<div class="map_content"> <p><span>м. Ленинский проспект</span></p> <div class="line"></div><p>Санкт-Петербург, Ленинский проспект, д. 97</p><p><a href="tel:+78124261015"><span>+7 (812) 426-10-15</span></a></p><div class="line"></div><p>пн-сб 10.00-20.00<br> воскресенье 10.00-18.00</p></div>'
            },
            {
                iconLayout: 'default#image',
        ...
        var myPlacemark2 = new ymaps.Placemark([59.852384, 30.313640], {
                balloonContent: '<div class="map_content"> <p><span>м. Московская</span></p> <div class="line"></div><p>Санкт-Петербург, ул. Варшавская, д. 59</p><p><a href="tel:+78124261015"><span>+7 (812) 426-10-15</span></a></p><div class="line"></div><p>пн-сб 10.00-20.00<br> воскресенье 10.00-18.00</p> </div>'
            },
            {
                iconLayout: 'default#image',
                iconImageHref: 'frontend/images/marker.png',
        ...
        var myPlacemark3 = new ymaps.Placemark([60.002148, 30.385506], {
                balloonContent: '<div class="map_content"> <p><span>м. Академическая</span></p> <div class="line"></div><p>Санкт-Петербург, Гражданский пр., д. 18</p><p><a href="tel:+78124261015"><span>+7 (812) 426-10-15</span></a></p><div class="line"></div><p>пн-сб 10.00-20.00<br> воскресенье 10.00-18.00</p> </div>'
            },
            {
                iconLayout: 'default#image',
                iconImageHref: 'frontend/images/marker.png',
        ...
        var myPlacemark4 = new ymaps.Placemark([59.906300, 30.479119], {
                balloonContent: '<div class="map_content"> <p><span>м. Дыбенко</span></p> <div class="line"></div><p>Санкт-Петербург, ул. Дыбенко, д. 24</p><p><a href="tel:+78124261015"><span>+7 (812) 426-10-15</span></a></p><div class="line"></div><p>пн-сб 10.00-20.00<br> воскресенье 10.00-18.00</p> </div>'
            },
            {
                iconLayout: 'default#image',
                iconImageHref: 'frontend/images/marker.png',
        ...
        var myPlacemark5 = new ymaps.Placemark([59.832059, 30.386943], {
                balloonContent: '<div class="map_content"> <p><span>м. Купчино</span></p> <div class="line"></div><p>Санкт-Петербург, ул. Ярослава Гашека, д. 4</p><p><a href="tel:+78124261015"><span>+7 (812) 426-10-15</span></a></p><div class="line"></div><p>пн-сб 10.00-20.00<br> воскресенье 10.00-18.00</p> </div>'
            },
            {
                iconLayout: 'default#image',
                iconImageHref: 'frontend/images/marker.png',
        ...
        var myPlacemark6 = new ymaps.Placemark([60.035841, 30.415743], {
                balloonContent: '<div class="map_content"> <p><span>м. Гражданский  проспект</span></p> <div class="line"></div><p>Санкт-Петербург, пр. Просвещения, д. 87</p><p><a href="tel:+78124261015"><span>+7 (812) 426-10-15</span></a></p><div class="line"></div><p>пн-сб 10.00-20.00<br> воскресенье 10.00-18.00</p> </div>'
            },
            {
                iconLayout: 'default#image',
                iconImageHref: 'frontend/images/marker.png',
        ...
        var myPlacemark7 = new ymaps.Placemark([60.010079, 30.263173], {
                balloonContent: '<div class="map_content"> <p><span>м. Комендантский  проспект</span></p> <div class="line"></div><p>Санкт-Петербург, ул. Уточкина, д. 3, кор.1</p><p><a href="tel:+78124261015"><span>+7 (812) 426-10-15</span></a></p><div class="line"></div><p>пн-сб 10.00-20.00<br> воскресенье 10.00-18.00</p> </div>'
            },
            {
                iconLayout: 'default#image',
                iconImageHref: 'frontend/images/marker.png',
        ...
        var myPlacemark8 = new ymaps.Placemark([59.834243, 30.199285], {
                balloonContent: '<div class="map_content"> <p><span>м. Проспект  Ветеранов</span></p> <div class="line"></div><p>Санкт-Петербург, пр. Ветеранов, д. 109</p><p><a href="tel:+78124261015"><span>+7 (812) 426-10-15</span></a></p><div class="line"></div><p>пн-сб 10.00-20.00<br> воскресенье 10.00-18.00</p></div>'
            },
            {
                iconLayout: 'default#image',
                iconImageHref: 'frontend/images/marker.png',
    ...
    function init_big_map() {
        myMap = new ymaps.Map("big_map", {
            ...
            center: [55.768993, 37.591845],
            zoom: 12,
            controls: []
        });

        var myPlacemark = new ymaps.Placemark([55.788515, 37.567482], {
                balloonContent: '<div class="map_content"><p><span>м. Динамо, м. Петровский парк</span></p> <div class="line"></div><p>Ленинградский проспект д.36, стр.10 "Арена парк"</p><p><a href="tel:+74951458118"><span>+7 (495) 145-81-18</span></a></p><div class="line"></div><p>пн-сб 9.00-19.00 <br> сб 10.00-18.00</p> </div>'
            },
            {
                iconLayout: 'default#image',
                iconImageHref: 'frontend/images/marker/marker.png',
        ...
        var myPlacemark1 = new ymaps.Placemark([55.768993, 37.591845], {
                balloonContent: '<div class="map_content"> <p><span>м. Маяковская</span></p> <div class="line"></div><p>Большая садовая, д.5, к.1 "Сады Пекина"</p><p><a href="tel:+74951458118"><span>+7 (495) 145-81-18</span></a></p><div class="line"></div><p>пн-сб 9.00-19.00 <br> сб 10.00-18.00</p> </div>'
            },
            {
                iconLayout: 'default#image',
                iconImageHref: 'frontend/images/marker/marker.png',
        ...
        var myPlacemark2 = new ymaps.Placemark([55.762151, 37.656703], {
                balloonContent: '<div class="map_content"> <p><span>м. Курская, м. Чкаловская</span></p> <div class="line"></div><p>Земляной вал, д.8 "Земляной вал"</p><p><a href="tel:+74951458118"><span>+7 (495) 145-81-18</span></a></p><div class="line"></div><p>пн-сб 9.00-19.00 <br> сб 10.00-18.00</p> </div>'
            },
            {
                iconLayout: 'default#image',
                iconImageHref: 'frontend/images/marker/marker.png',
    ...
    $("#popup_portfolio_slider .popup_block").on("click", ".all_slider .slider_info_portfolio .foto_menu ul li a:not(.link_page)", function (e) {
        e.preventDefault();
        var $this = $(this);
        var index = $this.parent().index();
    ...
    $("#popup_portfolio_slider .popup_block").on('click','.img_slid_list' ,function () {
        var $this = $(this);
        $this.parents(".slider_portfolio").find(".slider_portfolio_images_list .img_slid_list").removeClass("active");
        $this.addClass("active");
        $this.parents(".slider_portfolio").find(".owl-carousel").trigger('to.owl.carousel', $this.index());
    });
    ...
    function popupSliderScroll() {
        if ($(window).width() > 1100) {
            if ($(".slider_portfolio_images_list").hasClass("scroll_content")) {
    ...
    $(document).on("click", ".show_img_popup", function (e) {
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                $('.maska_phone').mask('+9 (999) 999-99-99').click(function () {
                    $(this).focus();
                });
            }
        });
    ...
    * */
    ...
    function new_comentsToggle() {
        if ($(window).innerWidth() < 820) {
            $("section.all_comments .all_comments_blocks .comment_blocks .block .new_comment > .block_title").on("click", function (e) {
                e.preventDefault();
                var $this = $(this);
    ...
    $("section.all_comments .all_comments_blocks .comment_blocks .block .new_comment form .sumbit_send .send_comm").on("click", function (e) {
        ...
        $.ajax({
            ...
            success: function () {
                ...
                $form.find('input[type="text"],input[type="file"], textarea').each(function (item, index) {
                    $(this).val('');
                });
                ...
                if (window.innerWidth > 1000) {
                    $('html, body').animate({
                        scrollTop: ($("section.all_comments .all_comments_blocks .comment_blocks .block .new_comment .thanks_comment").offset().top - 200)
                    }, 400);
                } else if (window.innerWidth < 820) {
                    $('html, body').animate({
                        scrollTop: ($("section.all_comments .comment_blocks").offset().top - 300)
                    }, 400);
                } else {
                    $('html, body').animate({
                        scrollTop: ($("section.all_comments .all_comments_blocks .comment_blocks .block .new_comment .thanks_comment").offset().top - 100)
                    }, 400);
                }
            ...
            error: function (response) {
                ...
                if (window.innerWidth > 1000) {
                    $('html, body').animate({
                        scrollTop: ($("section.all_comments .all_comments_blocks .comment_blocks .block .new_comment .new_comments_block").offset().top - 200)
                    }, 400);
                } else if (window.innerWidth < 820) {
                    $('html, body').animate({
                    ...
                    $('html, body').animate({
                        scrollTop: ($("section.all_comments .all_comments_blocks .comment_blocks .block .new_comment .new_comments_block").offset().top - 100)
                    }, 400);
                }
            }
        });
    ...
    $("header .logo_menu_mob .logo_menu_icons .phone_gen_menu .gen_menu_icons_base").on("click", function (e) {
        e.preventDefault();
        $("header .logo_menu_mob .show_menu").stop().slideDown(300);
    });

    $("header .logo_menu_mob .show_menu .close_menu_base").on("click", function (e) {
        e.preventDefault();
        $(this).hide();
    ...
    $("header .logo_menu_mob .logo_menu_icons .phone_gen_menu .gen_menu_icons").on("click", function (e) {
        $(this).hide();
        $("header .logo_menu_mob .logo_menu_icons .phone_gen_menu .header_phone").hide();

        $("header .logo_menu_mob .logo_menu_icons .phone_gen_menu .close_menu").show();
        $("header .logo_menu_mob .logo_menu_icons").toggleClass( "region_menu_open", "region_menu_close" );
        e.preventDefault();
        $("header .logo_menu_mob .show_menu").stop().slideDown(300);

    });

    $("header .logo_menu_mob .logo_menu_icons .phone_gen_menu  .close_menu").on("click", function (e) {
        e.preventDefault();
        $(this).hide();
        $("header .logo_menu_mob .logo_menu_icons .phone_gen_menu .header_phone").show();
        $("header .logo_menu_mob .logo_menu_icons .phone_gen_menu .gen_menu_icons").show();
        $("header .logo_menu_mob .logo_menu_icons").toggleClass( "region_menu_open", "region_menu_close" );
        $("header .logo_menu_mob .show_menu").stop().slideUp(300);
    ...
    $('.agree_call,#agree_call').on("change", function () {
        if (checkValidation()) {
    ...
    $("#popup_add_comment .popup_block").on("click", function (e) {
        e.stopPropagation();
    });

    $(".open_popup_add_comment").on("click", function (e) {
        e.preventDefault();
        $("body").css("overflow", "hidden");
        $("#popup_add_comment").fadeIn(function () {

        });
    });

    $("#popup_add_comment .close, #popup_add_comment").on("click", function (e) {
        e.preventDefault();
        $("#popup_add_comment").fadeOut(function () {
            $("body").css("overflow", "auto");
        });
    });
    ...
    $('#agree_letter').on("change", function () {
        var $this = $(this);
        if ($this.is(":checked")) {
            $this.parents('form').find('.submit').addClass('active');
        } else {
            $this.parents('form').find('.submit').removeClass('active');
        }
    });

    $("section.letter_director .form_block .flex_block .inp_block .inp_text,section.letter_director .form_block .textarea_inp textarea").on("keyup", function () {
        var $this = $(this);
        $this.parent().addClass("active_true");
        $this.parent().removeClass("active_false");
    });

    $("section.letter_director .form_block .flex_block .inp_block .inp_text,section.letter_director .form_block .textarea_inp textarea").on("blur ", function () {
        var $this = $(this);
        if ($this.val().length > 0) {
            if ($this.hasClass('emile_field') && !isEmail($this.val())) {
                $this.parent().addClass("active_false");
            } else {
                $this.parent().removeClass("active_true");
                $this.parent().removeClass("active_false");
            }
        } else {
            $this.parent().addClass("active_false");
    ...
    function header_fix () {
        if($(window).width() > 1000){
            if($(window).scrollTop() >= $("#scrolHeader .menu_top").innerHeight()){
                $("#scrolHeader").addClass("fixed_scroll");
            } else{
                $("#scrolHeader").removeClass("fixed_scroll");
            }
        }else{
            $("#scrolHeader").removeClass("fixed_scroll");
        }
    ...
    /*$ ( 'section.client_feedback_n .client_feedback_owl' ).owlCarousel ( {
      margin : 24 ,
      navText : false ,
      items : 2 ,
      nav : true ,
    ...
    function clientFeedbackSlider() {
        var new_slider_banner = $ ( 'section.client_feedback_n .client_feedback_owl' );
        if ($(window).width() > 640) {
            new_slider_banner.owlCarousel({
                margin : 24 ,
                navText : false ,
                items : 2 ,
                nav : true ,
            ...
            new_slider_banner.trigger('destroy.owl.carousel').removeClass('owl-carousel owl-loaded');
            new_slider_banner.find('.owl-stage-outer').children().unwrap();

        }
    }

    $ ( 'section.tur_3d_n .tur_3d_n-owl' ).owlCarousel ( {
        margin : 0 ,
        items : 1 ,
        nav : true ,
        loop:true,
        navText: ["","Следующий тур"]
    } );

    $ ( "section.cost_repair .how_much_cost--block-click" ).on ( "click" , function () {
        var $this = $(this);
        $this.toggleClass("active");
        $this.parents(".how_much_cost--block").find(".how_much_cost--block-content").stop().slideToggle(300);
    } );
    ...
    rightBlockSellFixed();
    function  rightBlockSellFixed (  ) {
        if ( $(window).width() >= 767 ) {
            /*var block_fixed = $("section.calculate_n .calculate_n--end .calculate_n--end--all").offset().top;
            var block_offset = $("section.calculate_n .calculate_n--end ").offset().top;
            var block_height_fight = $("section.calculate_n .calculate_n--end .calculate_n--end--all").height();
            var next_section =  $("section.calculate_n").next("section").offset().top;*/


            if($("section.calculate_n .calculate_n--end .calculate_n--end--all").hasClass("calculate_n--end--all")){
                var block_fixed = $("section.calculate_n .calculate_n--end .calculate_n--end--all").offset().top;
                var block_offset = $("section.calculate_n .calculate_n--end ").offset().top;
                var block_height_fight = $("section.calculate_n .calculate_n--end .calculate_n--end--all").height();
                var next_section =  $("section.calculate_n").next("section").offset().top;
                if($(window).scrollTop() > block_offset - 115 + 224 && $(window).scrollTop() <  next_section - block_height_fight  - 240){
                    $("section.calculate_n .calculate_n--end .calculate_n--end--all").addClass("active_fixed");
                    $("section.calculate_n .calculate_n--end .calculate_n--end--all").removeClass("active_absolute");
                } else{
                    $("section.calculate_n .calculate_n--end .calculate_n--end--all").removeClass("active_fixed");
                    if ( $(window).scrollTop() >  next_section - block_height_fight  - 240 ) {

                        $("section.calculate_n .calculate_n--end .calculate_n--end--all").addClass("active_absolute");
                    }
    ...
    usefulMaterialsMore();
    function usefulMaterialsMore(){
        if ( $(window).width() <= 640 ){
            $("section.client_feedback_n .client_feedback_owl .client_feedback_block").removeClass("active");
            $("section.client_feedback_n .client_feedback_owl .client_feedback_block").first().addClass("active");
        }else{
            $("section.client_feedback_n .client_feedback_owl .client_feedback_block").removeClass("active");
        }
    }
    $ ( "section.made_repairs .made_repairs_btn .btn_style_n-big" ).on ( "click" , function (e) {
        e.preventDefault ();
        var show_item = 3;
        if ($("section.made_repairs .made_repairs--blocks .made_repairs--block.active").length + 3 >= $("section.made_repairs .made_repairs--blocks .made_repairs--block").length   ){
            $ ( "section.made_repairs .made_repairs_btn .btn_style_n-big" ).hide();
            $ ( "section.made_repairs .made_repairs_btn .portfolio_link_v").show();
        }
        $('section.made_repairs .made_repairs--blocks .made_repairs--block:not(.active):lt(' + show_item + ')').addClass("active");
        
    } );

    
    $ ( "section.client_feedback_n  .view_all a.read_more" ).on ( "click" , function (e) {
        e.preventDefault ();
        var last_active =$("section.client_feedback_n .client_feedback_owl .client_feedback_block.active").last().index();

        if ($("section.client_feedback_n .client_feedback_owl .client_feedback_block").last().hasClass("active")){
            $ ( "section.client_feedback_n  .view_all a.read_more" ).hide();
        } else{
            $("section.client_feedback_n .client_feedback_owl .client_feedback_block").eq(last_active + 1).addClass("active");
        }
    } );
    ...
    $ ( "#popup_video_block .popup_block" ).on ( "click" , function ( e ) {
        e.stopPropagation ();
    } );

    $ ( ".open_video_popup" ).on ( "click" , function ( e ) {
        e.preventDefault ();
        $ ( "body" ).css ( "overflow" , "hidden" );
        var $this = $(this);
        $ ( "#popup_video_block .video_block" ).html('<iframe src="https://www.youtube.com/embed/' + $this.parent().find("img").data("video") + '?autoplay=1" allowfullscreen></iframe>');
        $ ( "#popup_video_block" ).fadeIn ( function () {

        } );
    } );

    $ ( "#popup_video_block .close, #popup_video_block " ).on ( "click" , function ( e ) {
        e.preventDefault ();
        $ ( "#popup_video_block" ).fadeOut ( function () {
            $ ( "body" ).css ( "overflow" , "auto" );
            $ ( "#popup_video_block .video_block" ).empty();
        } );
    } );

    $ ( "#popup_call--new .popup_block" ).on ( "click" , function ( e ) {
        e.stopPropagation ();
    } );

    $ ( ".call_open_popup_button--new" ).on ( "click" , function ( e ) {
        e.preventDefault ();
        $ ( "body" ).css ( "overflow" , "hidden" );
        $ ( "#popup_call--new" ).fadeIn ( function () {

        } );
    } );

    $ ( "#popup_call--new .close, #popup_call--new " ).on ( "click" , function ( e ) {
        e.preventDefault ();
        $ ( "#popup_call--new" ).fadeOut ( function () {
            $ ( "body" ).css ( "overflow" , "auto" );
        } );
    } );
    ...
    if ($("div").hasClass("scroll_content_feedback")) {
        $(".scroll_content_feedback").mCustomScrollbar({
            axis: "y",

        });
    }

    $ ( "section.cost_repair .view_text_all a" ).on ( "click" , function (e) {
        e.preventDefault ();
        var $this  =$(this);
        if ( $this.parents().hasClass("active") ){
            $this.text("Читать далее");
            $this.parents().removeClass("active");
            $this.parents("section.cost_repair").find(".cost_repair--text_block .flex_block_n").removeClass("active");
        }else{
            $this.text("Скрыть");
            $this.parents().addClass("active");
            $this.parents("section.cost_repair").find(".cost_repair--text_block .flex_block_n").addClass("active");
        }
    } );
    $(function(){
        $('.form_calc span#hide').text($('#calc_count_input').val());
        $('#calc_count_input').width($('.form_calc span#hide').width() + 5 +"px");
    }).on('change','', function () {
        $('.form_calc span#hide').text($('#calc_count_input').val());
        $('#calc_count_input').width($('.form_calc span#hide').width() + 5 +"px");
    });

    function updateSumCalc(){
        $('.form_calc span#hide').text($('#calc_count_input').val());
        $('#calc_count_input').width($('.form_calc span#hide').width() + 5 +"px");
    }

    $(".open_text_popup_call").on('click', function (e) {
        e.preventDefault();
        var $this = $(this);
        $this.parents(".popup_block").find(".all_text_scrolling").slideToggle();
    });
...
function initRepairCarousels() {
    ...
    if ($('#popup_portfolio_slider .slider_portfolio_6 .img_slid').length) {
        $('#popup_portfolio_slider .slider_portfolio_6').owlCarousel({
            margin: 0,
            navText: false,
            items: 1,
            nav: true,
            loop: $('#popup_portfolio_slider .slider_portfolio_6 .img_slid').length > 1,
            onInitialized: function (e) {
                $('#popup_portfolio_slider .popup_block .all_slider .slider_info_portfolio .img_numbers .number_6').text('(фото 1 из ' + this.items().length + ')');
            },
            responsive: {
                0: {
                    // autoHeight:true,
    ...
    $(".call_open_blog_popup_button").on("click", function (e) {
        var title;
        ...
        var tag = $(this).data('tag');
        if (tag && tag.length) title = tag;

        yaCounter44756221.reachGoal('blogg');
        ga('send', 'event', 'Heder', 'blogg');
    ...
    /*
      ...
      *
     */
    $("#popup_call .button_type,#popup_call--new .button_type").on("click", function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (typeof yandexMetrica == 'undefined'){
                    if (type === 'design'){
                        yandexMetrica  = 'СТА_Дизайн'
                    } else if (type === 'repairs'){
                        yandexMetrica  = 'СТА_Замер'
                    } else {
                        yandexMetrica = 'Хедер_заявка';
                    }
                }
...
function initPortfolioNumberChange() {
    $('#popup_portfolio_slider .slider_portfolio_1').on('changed.owl.carousel', function (e) {
        ...
        $('#popup_portfolio_slider .slider_portfolio_1').parent().find(' .slider_portfolio_images_list .img_slid_list').removeClass("active");
        $('#popup_portfolio_slider .slider_portfolio_1').parent().find('  .slider_portfolio_images_list .img_slid_list').eq(e.page.index - 1).addClass("active");
    });
    $('#popup_portfolio_slider .slider_portfolio_2').on('changed.owl.carousel', function (e) {
        ...
        $('#popup_portfolio_slider .slider_portfolio_2').parent().find('  .slider_portfolio_images_list .img_slid_list').removeClass("active");
        $('#popup_portfolio_slider .slider_portfolio_2').parent().find('  .slider_portfolio_images_list .img_slid_list').eq(e.page.index - 1).addClass("active");
    });
    $('#popup_portfolio_slider .slider_portfolio_3').on('changed.owl.carousel', function (e) {
        ...
        $('#popup_portfolio_slider .slider_portfolio_3').parent().find('  .slider_portfolio_images_list .img_slid_list').removeClass("active");
        $('#popup_portfolio_slider .slider_portfolio_3').parent().find('  .slider_portfolio_images_list .img_slid_list').eq(e.page.index - 1).addClass("active");
    ...
    $('#popup_portfolio_slider .slider_portfolio_4').on('changed.owl.carousel', function (e) {
        ...
        $('#popup_portfolio_slider .slider_portfolio_4').parent().find('  .slider_portfolio_images_list .img_slid_list').removeClass("active");
        $('#popup_portfolio_slider .slider_portfolio_4').parent().find('  .slider_portfolio_images_list .img_slid_list').eq(e.page.index - 1).addClass("active");
    });

    $('#popup_portfolio_slider .slider_portfolio_6').on('changed.owl.carousel', function (e) {

        $('#popup_portfolio_slider .popup_block .all_slider .slider_info_portfolio .img_numbers .number_6').text('(фото ' + ++e.page.index + ' из ' + e.item.count + ')');
        $('#popup_portfolio_slider .slider_portfolio_6').parent().find('  .slider_portfolio_images_list .img_slid_list').removeClass("active");
        $('#popup_portfolio_slider .slider_portfolio_6').parent().find('  .slider_portfolio_images_list .img_slid_list').eq(e.page.index - 1).addClass("active");
    });
}
...
function showItemPortfolio() {
    ...
    $("body").on("click", "section.all_portfolio .show_more a", function (e) {
        ...
        var lastItem = $('section.all_portfolio .portfolio_block .block:last-child');
        var pos = lastItem.position().top;

        console.log('portfolio_block');
...
$("body").on("click", ".reviews__hide a.show__more", function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (!response.hasCount) {
                $('.reviews__hide a.show__more').remove();
            }

            let oldHeight = $('.all_comments_blocks.reviews__hide').innerHeight();

            $('.all_comments_blocks.reviews__hide').masonry('destroy').masonry({
                // options
                itemSelector: '.block',
                columnWidth: '.block',
                gutter: 20
            ...
            $('html, body').animate({
                scrollTop: (oldHeight - 180)
            }, 0);


            // $('.all_comments_blocks.reviews__hide').css('height',$('.all_comments_blocks.reviews__hide #mob_review_block').innerHeight())
        },
        error: function (error) {
            console.log(error);
        }
...
$("body").on("click", ".show_more_block", function (e) {
    e.preventDefault();
    let step = $(this).data('step');
    let category = $(this).data('category');
    $(this).data('step', step + 1);
    $.ajax({
        url: '/blog/show-more-block',
        data: {
            step: step,
            category: category,
        },
        ...
        dataType: "json",
        success: function (response) {
            $('.posts-' + category).append(response.html);
            if (!response.hasCount) {
                $('.posts-' + category).parent().find('.show_more_block').remove();
            }
        },
        error: function (error) {
            console.log(error);
...
$("body").on("click", ".show-more-block-widget", function (e) {
    e.preventDefault();
    let step = $(this).data('step');
    let category = $(this).data('category');
    $(this).data('step', step + 1);
    ...
    var lastItem = $('.all_materials_blocks .useful_materials--block:last-child');
   //console.log(lastItem);
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (response) {
            //добавляем прошлым записям класс который в моб.версии невидим

            //$('.show-more-block-widget').parent().find('.all_materials_blocks').find('.useful_materials--block').addClass("mobile_hide");

            $('.show-more-block-widget').parent().find('.all_materials_blocks').append(response.html);
            if (!response.hasCount) {
                $('.show-more-block-widget').remove();
            }
            var mheight = $('.show-more-block-widget').parent().find('.all_materials_blocks').height();
            //console.log(pos);
            $('html,body').scrollTop(pos);
        },
        error: function (error) {
...
$("body").on("click", ".show_more_category", function (e) {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (response) {
            $('#category-materials').append(response.html);
            if (!response.hasCount) {
                $('.show_more_category').remove();
            }
        },
        error: function (error) {
            console.log(error);
...
$("body").on("click", ".mob__reviews__hide a.show__more", function (e) {
    e.preventDefault();
    let step = $(this).data('step');
    $(this).data('step', step + 1);
    $.ajax({
        ...
        dataType: "json",
        success: function (response) {
            let oldHeight = $('.mob__reviews__hide a.show__more').innerHeight();

            $('#mob_review_block_mob').append(response.html);
            if (!response.hasCount) {
                $('.mob__reviews__hide a.show__more').remove();
            }
            console.log(oldHeight, e.pageX, e.pageY);
            $('html, body').animate({
                scrollTop: (e.pageY-70)
            }, 0);
...
$("section.reviews_new .slider_reviews_new").owlCarousel({
    loop: true,
    items: 3,
    nav: true,
    navText: false,
    ...
    onTranslate: Owl_dot_onTranslate,

});

function Owl_dot_onTranslate() {
    var dot_lenght = $("section.reviews_new .slider_reviews_new").find(".owl-dot").length;
    $("section.reviews_new .slider_reviews_new").find(".owl-dot").removeClass("active_show");
    $("section.reviews_new .slider_reviews_new").find(".owl-dot").removeClass("active_show");
    $("section.reviews_new .slider_reviews_new").find(".owl-dot.active").addClass("active_show");
    $("section.reviews_new .slider_reviews_new").find(".owl-dot.active").next().addClass("active_show");
    $("section.reviews_new .slider_reviews_new").find(".owl-dot.active").next().next().addClass("active_show");
    $("section.reviews_new .slider_reviews_new").find(".owl-dot.active").prev().addClass("active_show");
    $("section.reviews_new .slider_reviews_new").find(".owl-dot.active").prev().prev().addClass("active_show");
    if ($("section.reviews_new .slider_reviews_new").find(".owl-dot.active").prev().hasClass("active_show")) {

    } else {
        for (var $i = 0; $i <= 4; $i++) {
            $("section.reviews_new .slider_reviews_new").find(".owl-dot").eq($i).addClass('active_show');
        }
    }
    if ($("section.reviews_new .slider_reviews_new").find(".owl-dot.active").prev().prev().hasClass("active_show")) {
    ...
    if ($("section.reviews_new .slider_reviews_new").find(".owl-dot.active").next().hasClass("active_show")) {

    } else {
        $("section.reviews_new .slider_reviews_new").find(".owl-dot").eq(dot_lenght - 4).addClass('active_show');

    }
    if ($("section.reviews_new .slider_reviews_new").find(".owl-dot.active").next().next().hasClass("active_show")) {

    } else {
        $("section.reviews_new .slider_reviews_new").find(".owl-dot").eq(dot_lenght - 4).addClass('active_show');
        $("section.reviews_new .slider_reviews_new").find(".owl-dot").eq(dot_lenght - 5).addClass('active_show');
    }


}

Owl_dot_onInitialized();

function Owl_dot_onInitialized() {
...
var owl_review = $("section.reviews_new .slider_reviews_new").data('owlCarousel');

$("body").on("click", '.reviews_new .owl-item.active', function (e) {
...
//         var new_x = Math.round((x.replace(/[^0-9]/g, '')) * 1.087 * 1.15);
//
//         $("section.price_list_new_page .all_price_list .price_list_block").eq(z).find(".name_with_block .info_price .all_table table tbody > tr").eq(i).find(" > td table tr td:first-child").html("<span>" + Math.round(new_x) + "  руб.</span> " + Math.round(new_x * 0.85) + " руб.");
...
//             var new_val_th = Math.round((val_th.replace(/[^0-9]/g, '')) * 1.087 * 1.15 * 0.85);
//
...
$(window).on("resize scroll", function () {
    clientInteriorSlider();

});

clientInteriorSlider();

function clientInteriorSlider() {
    var new_slider_int = $ ( 'section.interior_design .interior_design__slider' );
    if ($(window).width() > 640) {
        new_slider_int.owlCarousel({
            margin:40,
            navText : false ,
            items : 2 ,
            nav : true ,
        ...
        new_slider_int.trigger('destroy.owl.carousel').removeClass('owl-carousel owl-loaded');
        new_slider_int.find('.owl-stage-outer').children().unwrap();

    }
}

$ ( "section.interior_design  .view_all a.read_more" ).on ( "click" , function (e) {
    e.preventDefault ();

    var last_active =$("section.interior_design .interior_design__slider .interior_design__slider-item.active").last().index();
    if ($("section.interior_design .interior_design__slider .interior_design__slider-item").last().hasClass("active")){
        $ ( "section.interior_design  .view_all a.read_more" ).hide();
    } else{
        $("section.interior_design .interior_design__slider .interior_design__slider-item").eq(last_active + 1).addClass("active");
    }
} );

$(window).on(" resize", function () {
    interiorMaterialsMore();
});


interiorMaterialsMore();
function interiorMaterialsMore(){
    if ( $(window).width() <= 640 ){
        $("section.interior_design .interior_design__slider .interior_design__slider-item").removeClass("active");
        $("section.interior_design .interior_design__slider .interior_design__slider-item").first().addClass("active");
        $("section.interior_design .interior_design__slider .interior_design__slider-item").first().next().addClass("active");
    }else{
        $("section.interior_design .interior_design__slider .interior_design__slider-item").removeClass("active");
    }
}

$("section.your_portfolio_home_page_new .show_all_link_a .show_all").on("click", function (e) {

    var lastItem = $('.your_portfolio_home_page_blocks .portfolio_block:last-child');
    //console.log(lastItem);
    var pos = lastItem.position().top;

    e.preventDefault();
    var $this = $(this);
    $("section.your_portfolio_home_page .show_all_link_a .show_all").hide();
    $("section.your_portfolio_home_page .show_all_link_a .link_a").addClass("show_a");
    $("section.your_portfolio_home_page .your_portfolio_home_page_blocks .portfolio_block").addClass("active");
    $('html,body').scrollTop(pos);
...
$ ( "#popup_services_table .popup_block .popup__tables .table__tr .table__td .table__tariff" ).on ( "click" , function () {

    var $this = $(this);
    var parent__index = $this.parent(".table__td").index();
    $("#popup_services_table .popup_block .popup__tables .table__tr .table__td").removeClass("active");
    for ( var i = 0; i <$("#popup_services_table .popup_block .popup__tables .table__tr").length; i++  ) {
        $("#popup_services_table .popup_block .popup__tables  .table__tr ").eq(i).find(".table__td").eq(parent__index).addClass("active");
    }
} );

if ($(".scroll_content_int").hasClass("scroll_content_int")) {
    $(".scroll_content_int").mCustomScrollbar({
        axis: "y",
